...
jQuery.fn.timelinr = function(options){
	...
	settings = jQuery.extend({
		...
		liCADMO: 					'listasAnidadas',	// value: clase para listas anidadas
	...
	$(function(){
		...
		var howManyIssues = $(settings.issuesDiv+' li.'+settings.liCADMO).length;
		...
		var widthIssue = $(settings.issuesDiv+' li.'+settings.liCADMO).width();
		var heightIssue = $(settings.issuesDiv+' li.'+settings.liCADMO).height();
		...
		$(settings.datesDiv+' a').click(function(event){
			...
			$(settings.issuesDiv+' li.'+settings.liCADMO).animate({'opacity':settings.issuesTransparency},{queue:false, duration:settings.issuesSpeed}).removeClass(settings.issuesSelectedClass).eq(currentIndex).addClass(settings.issuesSelectedClass).fadeTo(settings.issuesTransparencySpeed,1);
			...
			if(settings.orientation == 'horizontal') {
				...
				if(currentPositionIssues <= -(widthIssue*howManyIssues-(widthIssue))) {
					...
					if (!$(settings.issuesDiv).is(':animated')) {
						widthIssue = $(settings.issuesDiv+' li.'+settings.liCADMO).width();
						$(settings.issuesDiv).animate({'marginLeft':currentPositionIssues-widthIssue},{queue:false, duration:settings.issuesSpeed});
						$(settings.issuesDiv+' li.'+settings.liCADMO).animate({'opacity':settings.issuesTransparency},{queue:false, duration:settings.issuesSpeed});
